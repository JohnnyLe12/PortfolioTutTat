    ...
    <div className="container mx-auto px-6 py-8 max-w-4xl">
      ...
      {!loading && !error && applications.length > 0 && (
        <div className="mb-6 text-sm text-gray-600">
          {t("applications.totalApplications")}: <span className="font-semibold text-gray-900">{applications.length}</span>
        </div>
      )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                    <span>{t("applications.applied")}: {formatDate(application.createdAt)}</span>
                    {application.portfolioIds && application.portfolioIds.length > 0 && (
                      <span>
                        {application.portfolioIds.length} portfolio{application.portfolioIds.length > 1 ? "s" : ""} {t("applications.attached")}
                      </span>
                    )}
                  </div>
                </div>
